...
def developer_func(desarrollador:str):
    """
    Esta funcion calcula para un desarrollador de juego devuelve la cantidad de items y el porcentaje de contenido gratis por año
    params:
    desarrollador:str
    """
...
def user_recommend_fuc(user:str):
    ...
    if  not user_reviews['user_id'].eq(user).any():
        ...
        if  not user_item_max_horas['user_id'].eq(user).any():
            ...
            return item_recommend_func(int(item.iloc[0]))
